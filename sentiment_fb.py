...
# reading datasets
fb_data = pd.read_csv('D:/Program Files/Python/Social Media Analytics File/Assignment 2/com_fb/shark_comment.csv')
...
fb_token = fb['Value'].apply(word_tokenize).tolist()


cleaned_tokens = []
for tokens in fb_token:
...
fb['Sentiment'] = text_blob

polarity = []
subjectivity = []
for i, value in fb.iterrows():
    text = TextBlob(value['Value'])
...
labelled_fb_cmt = pd.concat([fb, c], axis=1)
...
# sentiment counts
# fb_sent_counts = fb['Sentiment'].value_counts()
# fb_sent_counts = pd.DataFrame(fb_sent_counts, columns=['Counts'])
m.to_csv(r'D:\Program Files\Python\Social Media Analytics File\Assignment 2\com_fb\results\shark_sent_average_fb.csv')
labelled_fb_cmt.to_csv(r'D:\Program Files\Python\Social Media Analytics File\Assignment 2\com_fb\results\shark_sent_fb.csv')