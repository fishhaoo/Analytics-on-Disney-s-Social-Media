...
# reading datasets
ig_master = pd.read_csv('D:/Program Files/Python/Social Media Analytics File/Assignment 2/com_ig/simpsons_ig_comment.csv')


# dataset cleaning 
ig = ig_master[['Comments']]
...
ig_token = ig['Comments'].apply(word_tokenize).tolist()


cleaned_tokens = []
for tokens in ig_token:
...
ig['Sentiment'] = text_blob

polarity = []
subjectivity = []
for i, value in ig.iterrows():
    text = TextBlob(value['Comments'])
...
labelled_ig_cmt = pd.concat([ig, c], axis=1)
...
# sentiment counts
# ig_sent_count = ig['Sentiment'].value_counts()
# ig_sent_counts = pd.DataFrame(ig_sent_count, columns=['Sentiment'])
m.to_csv(r'D:\Program Files\Python\Social Media Analytics File\Assignment 2\com_ig\results\simpsons_sent_average_ig.csv')
labelled_ig_cmt.to_csv(r'D:\Program Files\Python\Social Media Analytics File\Assignment 2\com_ig\results\simpsons_sent_ig.csv')